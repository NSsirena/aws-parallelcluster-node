...
class ClusterEventPublisher:
    ...
    #  @log_exception(logger, "publish_node_mapping_events", catch_exception=Exception, raise_on_error=False)
    def publish_node_mapping_events(self, nodes: List[SlurmNode]):
        """Publish events for node_mapping."""
        timestamp = ClusterEventPublisher.timestamp()
        nodes_with_instance = [node for node in nodes if node.instance]
        self.publish_event(
            logging.INFO,
            **NODE_INSTANCE_MAPPING_EVENT,
            timestamp=timestamp,
            event_supplier=self._node_mapping_supplier(nodes_with_instance),
        )
    ...
    @staticmethod
    def _node_mapping_supplier(nodes: List[SlurmNode]) -> Iterator:
        yield {
            "detail": {
                "node_list": [
                    {
